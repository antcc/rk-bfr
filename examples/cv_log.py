...
def cv_sk(
    classifiers, folds, X, Y, X_test, Y_test,
...
df_metrics_mcmc.sort_values(
    results_columns_mcmc[-1], inplace=True, ascending=False)
...
filename = ("clf_" + MCMC_ALG + "_" + data_name
            + "_noise_" + str(int(100*NOISE))
...
try:
    if SAVE_RESULTS and rep + 1 > 0:
        ...
        mcmc_best = df_metrics_mcmc["Accuracy"][:SAVE_TOP].apply(
        ...
        if FIT_REF_ALGS:
            sk_best = df_metrics_sk["Accuracy"][:SAVE_TOP].to_numpy(
                dtype=float)
        else:
            sk_best = np.zeros(1)
